...
export async function createCollection(data) {const response = await axios.post("create_collection", {name: data.collectionName, topic: data.topic, description: data.description, image: data.image, userId: data.userId}).then(res => {res.fields = data.fields; createCollectionFields(res)})}

export async function createCollectionFields(data) {console.log(data); const response = await axios.post("/create_fields", {collectionId: data.data.id, fields: data.fields}); return response.data;}
