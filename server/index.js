...
const {user, collection, field, tag} = require("../models");
...
app.post("/register_user", async(req, res) => {
   ...
   const newUser = await user.create({userName: userName, password: password, email: email, status: "active", permission_id: 2})
    try {res.send(newUser);}
...
app.post('/block_user',async(req,res)=>{
    ...
    await user.update({status: "blocked"},{ where : {id : req.body._id }});
});

app.post('/unblock_user',async(req,res)=>{
    console.log(req.body)
    await user.update({status: "active"},{ where : {id : req.body._id }});
});
...
app.post("/create_collection", async(req, res) => {
    ...
    const newCollection = await collection.create({
        name: name, topic: topic, description: description, image: image, userId: userId, itemsCount: 0})
    for (const field1 of fields) {
        const newField = await field.create({type: field1.type, name: field1.fieldName, collectionId: newCollection.id})
    }
    if(newCollection)
    {
...
app.get('/select_tags',async(req,res)=>{
...
app.post("/create_fields", async(req, res) => {
    ...
    for (const field1 of fields) {
        const newField= await field.create({type: field1.type, name: field1.fieldName, collectionId: id})
        try {
            res.send(newField);
        } catch (error) {
